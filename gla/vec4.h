...
namespace gla
{
    ...
    struct vec<4, T>
    {
        ...
        GLA_CONSTEXPR vec(T scalar) : x(scalar), y(scalar), z(scalar), w(scalar) { }

        GLA_CONSTEXPR vec(T x, T y, T z, T w) : x(x), y(y), z(z), w(w) { }

        static GLA_CONSTEXPR const std::size_t size() { return 4; }

        // ┌----------------------------------------------------┐
        // │    binary operators                                |
        // └----------------------------------------------------┘

        GLA_CONSTEXPR vec operator + (const vec &v) const { return { x + v.x, y + v.y, z + v.z, w + v.w }; }
        GLA_CONSTEXPR vec operator - (const vec &v) const { return { x - v.x, y - v.y, z - v.z, w - v.w }; }
        GLA_CONSTEXPR vec operator * (const vec &v) const { return { x * v.x, y * v.y, z * v.z, w * v.w }; }
        GLA_CONSTEXPR vec operator / (const vec &v) const { return { x / v.x, y / v.y, z / v.z, w / v.w }; }

        GLA_CONSTEXPR vec operator * (T scalar) const { return { x * scalar, y * scalar, z * scalar, w * scalar }; }
        GLA_CONSTEXPR vec operator / (T scalar) const { return { x / scalar, y / scalar, z / scalar, w / scalar }; }

        GLA_CONSTEXPR friend vec operator * (T scalar, const vec &v) { return { v.x * scalar, v.y * scalar, v.z * scalar, v.w * scalar }; }
        ...
        GLA_CONSTEXPR bool operator == (const vec &v) const { return x == v.x && y == v.y && z == v.z && w == v.w; }
        ...
        static GLA_CONSTEXPR vec cross(const vec &v0, const vec &v1)
        {
